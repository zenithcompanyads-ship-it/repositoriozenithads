...
export function generateCSVReport(data: CSVReportData): string {
  ...
  // Per-result-type breakdown for KPI strip
  const resultTypeMap = new Map<string, { label: string; costLabel: string; count: number; spend: number }>();
  for (const c of campaigns) {
    const rt = c.resultType ?? '';
    if (!resultTypeMap.has(rt)) {
      resultTypeMap.set(rt, {
        label: getResultLabel(c.resultType),
        costLabel: getResultCostLabel(c.resultType),
        count: 0,
        spend: 0,
      });
    }
    const e = resultTypeMap.get(rt)!;
    e.count += c.conversions;
    e.spend += c.spend;
  }
  const resultBreakdownArr = Array.from(resultTypeMap.values())
    .filter(e => e.count > 0 || e.spend > 0)
    .sort((a, b) => b.count - a.count)
    .slice(0, 3);
  const kpiColCount = resultBreakdownArr.length > 0 ? 3 + resultBreakdownArr.length * 2 : 5;

  // Hero summary
  const heroPeriodSummary = structured?.periodSummary
    ?? `Relatório ${numDays <= 7 ? 'semanal' : numDays <= 16 ? 'quinzenal' : 'mensal'} com ${campaigns.length} campanha${campaigns.length !== 1 ? 's' : ''} ativas entre ${fdateShort(periodStart)} e ${fdateShort(periodEnd)}. Investimento de ${brl(totalSpend)} gerando ${num(totalConversions)} ${resultLabel.toLowerCase()}.`;
  ...
  // Campaign sections
  const sortedByConv = [...campaigns].sort((a, b) => b.conversions - a.conversions || b.spend - a.spend);
  const top3 = sortedByConv.slice(0, 3);

  // Avg CPR for table coloring (only campaigns with conversions)
  const campsWithConv = campaigns.filter(c => c.conversions > 0);
  const avgCpr = campsWithConv.length > 0
    ? campsWithConv.reduce((sum, c) => sum + c.spend / c.conversions, 0) / campsWithConv.length
    : 0;
  ...
  const f3ToConv = audienciaResults > 0 ? ((totalConversions / audienciaResults) * 100).toFixed(1) : (totalReach > 0 ? ((totalConversions / totalReach) * 100).toFixed(2) : '0');
  ...
  // ── Emoji helpers ────────────────────────────────────────────────────────────
  function resultEmoji(rt: string | null | undefined): string {
    const r = (rt ?? '').toLowerCase();
    if (r.includes('messaging') || r.includes('onsite')) return '💬';
    if (r.includes('profile_visit')) return '👤';
    if (r.includes('lead')) return '📋';
    if (r.includes('purchase') || r.includes('compra')) return '🛒';
    if (r.includes('thruplay') || r.includes('video')) return '🎬';
    if (r.includes('reach') || r.includes('awareness')) return '📡';
    if (r.includes('link_click') || r.includes('click')) return '🖱️';
    return '🎯';
  }
  ...
  const css = `
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .nav-inner{display:flex;justify-content:space-between;align-items:center;padding:14px 28px;max-width:1120px;margin:0 auto}
    .nav-brand .nav-client{font-size:15px;font-weight:700;color:var(--white)}
    .nav-brand .nav-period{font-size:11px;color:var(--gray);margin-top:1px}
    .nav-right{font-size:11px;color:var(--gray)}
    ...
    .hero::after{content:'';position:absolute;bottom:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--gold),var(--blue),transparent)}
    ...
    .hero-logo-wrap{width:110px;height:110px;border-radius:50%;border:2px solid rgba(201,168,76,.25);background:rgba(201,168,76,.04);display:flex;align-items:center;justify-content:center;font-size:44px;box-shadow:0 0 60px rgba(201,168,76,.12);margin:0 0 14px auto}
  ...
  // ── Campaign Cards (Il Paradiso style) ──────────────────────────────────────
  ...
  function renderBigKpiCards(): string {
    const items = [
      { emoji: '🏆', val: num(totalReach), label: 'Pessoas Alcançadas', desc: `Presença de marca construída com ${campaigns.length} campanhas no período.`, color: 'var(--gold)' },
      { emoji: '📲', val: num(totalConversions), label: esc(resultLabel), desc: `${esc(resultLabel)} geradas com custo médio de ${cplGlobal > 0 ? brl(cplGlobal) : '—'} por resultado.`, color: 'var(--blue-l)' },
      { emoji: '💎', val: cplGlobal > 0 ? brl(cplGlobal) : '—', label: `Custo por ${esc(getResultCostLabel(globalResultType))}`, desc: `CPR médio no período — resultado de otimização contínua.`, color: 'var(--green)' },
      { emoji: '📡', val: num(totalImpressions), label: 'Impressões Totais', desc: `Frequência de ${frequency.toFixed(2)}x por pessoa — exposição de marca sólida.`, color: 'var(--orange)' },
    ];
  ...
  // ── Analysis: Positives & Opportunities (Il Paradiso) ─────────────────────
  function renderAnalysisGrid(): string {
    ...
    const posItems = positives.map(p => `
      <div class="anal-item">
  ...
  function renderMonthlySection(): string {
    ...
    const cpvFlow = `
      <div class="chart-card" style="margin-bottom:20px">
        <div class="chart-hdr">
          <div><div class="chart-title">Evolução do Custo por Resultado</div><div class="chart-sub">Acompanhamento mês a mês da eficiência das campanhas</div></div>
        </div>
        <div class="cpv-flow">
          ${cprByMonth.map((m, i) => `
      ...
      <div class="container">
        ...
        <div class="m-table-wrap chart-card">
          <div class="chart-hdr" style="margin-bottom:16px">
            <div><div class="chart-title">Comparativo Mensal Completo</div><div class="chart-sub">Todos os indicadores por mês</div></div>
          </div>
          <table class="m-table">
            <thead>
  ...
  // ── Build HTML (Il Paradiso style) ─────────────────────────────────────────
  return `<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Relatório de Performance · ${esc(clientName)}</title>
...
<nav>
  ...
  <div class="nav-right">
    <div class="hero-tag"><span class="dot"></span>Relatório Ativo</div>
...
<section class="hero-section">
  <div class="container">
    <div class="hero-grid">
      <div class="hero-left">
        <div class="sec-lbl gold">Relatório de Performance · Meta Ads</div>
        <h1 class="hero-title">Resultado das campanhas<br>de <em>${periodMonthName}.</em></h1>
        <p class="hero-sub">${esc(heroPeriodSummary)}</p>
      </div>
      <div class="hero-right">
        <div class="kpi-card primary">
          <div class="kpi-card-label">Investimento Total</div>
          <div class="kpi-card-val">${brl(totalSpend)}</div>
          <div class="kpi-card-sub">${esc(projectionHint)}</div>
        </div>
      </div>
    </div>
    ...
    <div class="kpi-grid">
      <div class="kpi-card gold-card">
        <div class="kpi-card-label">Impressões</div>
        <div class="kpi-card-val">${num(totalImpressions)}</div>
        <div class="kpi-card-sub">${campaigns.length} campanha${campaigns.length !== 1 ? 's' : ''} ativas</div>
      </div>
      <div class="kpi-card blue-card">
        <div class="kpi-card-label">Pessoas Alcançadas</div>
        <div class="kpi-card-val">${num(totalReach)}</div>
        <div class="kpi-card-sub">Freq. média ${frequency.toFixed(2)}x</div>
      </div>
      <div class="kpi-card green-card">
        <div class="kpi-card-label">${esc(resultLabel)}</div>
        <div class="kpi-card-val">${num(totalConversions)}</div>
        <div class="kpi-card-sub">CPR: ${cplGlobal > 0 ? brl(cplGlobal) : '—'}</div>
      </div>
    </div>

    <!-- RESULT TYPE BREAKDOWN -->
    ${resultBreakdownArr.length > 0 ? `
    <div class="kpi-grid-2">
      ${resultBreakdownArr.map((e, i) => {
        const cardClass = i === 0 ? 'gold-card' : i === 1 ? 'blue-card' : 'orange-card';
        return `
      <div class="kpi-card ${cardClass}">
        <div class="kpi-card-label">${esc(e.label)}</div>
        <div class="kpi-card-val">${num(e.count)}</div>
        <div class="kpi-card-sub">${e.count > 0 ? brl(e.spend / e.count) : '—'} por ${esc(e.costLabel)}</div>
      </div>`;
      }).join('')}
    </div>` : ''}
    ...
    <div class="strip">
      <div class="strip-item">
        <div class="strip-val">${brl(totalSpend / (numDays || 1))}</div>
        <div class="strip-lbl">Gasto / Dia</div>
      </div>
      <div class="strip-item">
        <div class="strip-val">${totalImpressions > 0 ? brl(totalSpend / totalImpressions * 1000) : '—'}</div>
      ...
      <div class="strip-item">
        <div class="strip-val">${campaigns.filter(c => isActiveCampaign(c.status)).length}</div>
...
<section class="section">
  <div class="container">
    <div class="sec-lbl blue">Visão Geral · ${campaigns.length} Campanha${campaigns.length !== 1 ? 's' : ''}</div>
    <div class="camp-grid">
      ${renderCampCards()}
    </div>
  </div>
...
<section class="section">
  <div class="container">
    ...
    <div class="two-col">
      <div class="funnel">
        <div class="f-step" style="width:100%">
          <span class="f-label">Pessoas Alcançadas</span>
          <span class="f-val">${num(totalReach)}</span>
        </div>
        <div class="f-pct">↓ ${reachToImpr}% freq. impressões</div>
        <div class="f-step" style="width:${Math.min(100, f2w).toFixed(0)}%">
          <span class="f-label">Impressões</span>
          <span class="f-val">${num(totalImpressions)}</span>
        </div>
        <div class="f-pct">↓ ${imprToF3}% engajamento</div>
        <div class="f-step" style="width:${Math.min(100, f3w).toFixed(0)}%">
          <span class="f-label">${audienciaResults > 0 ? 'Resultados Audiência' : 'Engajamento Est.'}</span>
          <span class="f-val">${audienciaResults > 0 ? num(audienciaResults) : '~' + num(Math.round(totalImpressions * 0.45))}</span>
        </div>
        <div class="f-pct">↓ ${f3ToConv}% conversão</div>
        <div class="f-step" style="width:${Math.max(8, Math.min(100, f4w)).toFixed(0)}%; background:var(--green)">
          <span class="f-label">${esc(resultLabel)}</span>
      ...
      <div class="chart-card">
        <div class="chart-hdr">
          <div>
            ...
            <div class="chart-sub">Custo por resultado de cada campanha</div>
          </div>
        </div>
        ${(() => {
          const sorted = [...campaigns].filter(c => c.conversions > 0).sort((a, b) => (a.spend/a.conversions) - (b.spend/b.conversions)).slice(0, 6);
          if (sorted.length === 0) return '<p style="color:var(--mid);font-size:0.8rem;padding:8px 0">Sem campanhas com resultados</p>';
          ...
          return sorted.map((c, i) => {
            ...
            return `
            <div class="bar-row">
              <div class="bar-label">${esc(c.name.length > 28 ? c.name.slice(0, 28) + '…' : c.name)}</div>
...
<section class="section">
  <div class="container">
    <div class="sec-lbl gold">Análise Estratégica · ${esc(periodMonthName)}</div>
...
<footer>
  <div class="logo">ZENITH<span>.</span></div>
  <p>Relatório gerado em ${todayFormatted} · Fonte: Meta Ads Manager · ${esc(clientName)}</p>
</footer>
