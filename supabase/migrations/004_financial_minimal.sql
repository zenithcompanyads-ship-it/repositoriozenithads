-- Financial Tables - Minimal Version
-- Only CREATE TABLE IF NOT EXISTS, no IF NOT EXISTS on policies
...
CREATE POLICY financial_categories_admin ON public.financial_categories FOR ALL USING (admin_id = auth.uid());

DROP POLICY IF EXISTS financial_clients_admin ON public.financial_clients;
CREATE POLICY financial_clients_admin ON public.financial_clients FOR ALL USING (admin_id = auth.uid());

DROP POLICY IF EXISTS financial_prospects_admin ON public.financial_prospects;
CREATE POLICY financial_prospects_admin ON public.financial_prospects FOR ALL USING (admin_id = auth.uid());
